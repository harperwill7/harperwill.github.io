...
#    2          Added setpoint validation and refined serial comms
#------------------------------------------------------------------
...
MIN_SETPOINT = 60
MAX_SETPOINT = 85
SERIAL_PORT = '/dev/ttyS0' # This would be /dev/ttyAM0 prior to Raspberry Pi 3
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_off(self):
        ##
        ## Change indicator lights states
        redLight.off()
        blueLight.off()
        if(DEBUG):
    ...
    def processTempIncButton(self):
        ...
        ##
        ## Update the setpoint and status lights, with bounds checking
        if self.setPoint < MAX_SETPOINT:
            self.setPoint += 1
    ...
    def updateLights(self):
        ...
        ##
        ## Update the status lights in the thermostat circuit
        if self.current_state.id == "heat":

            if temp < self.setPoint:
                redLight.pulse(fade_in_time=1, fade_out_time=1)
            else:
                redLight.value = 1  ## stays on, solid light
            ...
            if temp > self.setPoint:
                ...
                blueLight.value = 1  ## stays on, solid light
    ##
    ...
    def setupSerialOutput(self):
        ##
        ## Create the string assigned to the variable named output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## setup the first line of the LCD display to incude the current date and time.
            lcd_line_1 = current_time.strftime("%m/%d %H:%M:%S") + "\n"

            ## Setup Display Line 2
            if(altCounter < 6):

                ## setup the second line of the LCD display to incude the current temperature in degrees Fahrenheit
                lcd_line_2 = f"Temp:{floor(self.getFahrenheit())}F"
                altCounter = altCounter + 1

            else:

                ## setup the second line of the LCD display to incude the current state of the thermostat and the current temperature setpoint in degrees Fahrenheit.
            ...
            if((counter % 30) == 0):
                ...
                ## send our current state information to the TemperatureServer over the Serial Port (UART).
                ser.write(self.setupSerialOutput().encode())
                counter = 1 # Reset the counter only when a write occurs
            else:
                counter = counter + 1
            sleep(1)
...
## assign a function to be triggered when the button is pushed
...
## assign a function to be triggered when the button is pushed to increase the setpoint by one degree Fahrenheit.
...
## assign a function to be triggered when the button is pushed to decrease the setpoint by one degree Fahrenheit.
